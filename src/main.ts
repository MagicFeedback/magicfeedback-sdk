...
export default function main() {
    ...
    /**
     * Create a dynamic (AGENT mode) survey: the questions are generated by the
     * AI one turn at a time instead of being fetched upfront.
     *
     * This is the recommended entry point for agent surveys — it needs no
     * preliminary fetch. `form()` also auto-detects AGENT integrations, at the
     ...
     * @returns
     */
    function agentForm(integrationId: string, publicKey: string, privateKey?: string) {
        if (!integrationId) log.err("No integrationId provided");
        if (!publicKey) log.err("No publicKey provided");
        return new AgentForm(config, integrationId, publicKey, privateKey);
    }
